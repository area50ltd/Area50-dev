      ...
      <div className="max-w-6xl mx-auto px-6" ref={ref}>
        ...
        >
          ...
          <h2 className="font-heading text-4xl lg:text-5xl font-bold text-[#1B2A4A] mt-3 mb-4">
            Simple, Credit-Based{' '}
            <span className="text-[#E91E8C]">Pricing</span>
          </h2>
            ...
            >
              {plan.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <span className="bg-gradient-to-r from-[#E91E8C] to-[#FF6BB5] text-white text-xs font-bold px-4 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg">
                    <Zap size={11} />
                    Most Popular
                  </span>
                </div>
              ...
              <div className="mb-6">
                ...
                <div className={`text-xs mt-1 ${plan.highlight ? 'text-[#FF6BB5]' : 'text-[#E91E8C]'}`}>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((feat) => (
                  <li key={feat} className={`flex items-start gap-2.5 text-sm ${plan.highlight ? 'text-white/80' : 'text-neutral-600'}`}>
                    <Check size={15} className={`mt-0.5 flex-shrink-0 ${plan.highlight ? 'text-[#FF6BB5]' : 'text-[#E91E8C]'}`} />
                    {feat}
                  </li>
                ))}
              </ul>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {creditCosts.map((item) => (
              <div key={item.operation} className="text-center p-4 bg-neutral-50 rounded-xl">
                <div className="font-heading text-2xl font-bold text-[#E91E8C] mb-1">
                  {item.cost.split(' ')[0]}
                </div>
