...
const Schema = z.object({
  type: z.enum(['ai_message', 'human_message', 'voice_minute', 'outbound_call_flat', 'kb_embed']),
...
export async function POST(req: Request) {
  ...
  try {
    const result = await callN8n('/webhook/credits/deduct', {
      company_id: dbUser.company_id,
      ...parsed.data,
    })
    return NextResponse.json(result)
  } catch (err) {
    console.error('[api/credits/deduct]', err)
